...
class molarDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, data_file_list, training=True, root_path=None, logger=None):
        super().__init__(
        ...
        # import glob
        # data_file_list = glob.glob(str(Path(pcd_path + '/home/molar/code/ai_assist_be/pcdetModel/OpenPCDet/data/pc/*.pcd')))

        # data_file_list.sort()
        the_time = data_file_list[0]
        self.sample_sequence_list = [os.path.join('/home/molardata/dataset/data/{}/'.format(the_time), i['link']) for i in data_file_list[1]]
    ...
    def include_waymo_data(self, mode):
        ...
        #     info_path = self.data_path / sequence_name / ('%s.pcd' % sequence_name)
        ...
        self.infos.extend(self.sample_sequence_list)
    ...
    def __getitem__(self, index):
        ...
        # return data_dict
        if self.sample_sequence_list[index].endswith('.bin'):
            points = np.fromfile(self.sample_sequence_list[index], dtype=np.float32).reshape(-1, 4)
        elif self.sample_sequence_list[index].endswith('.npy'):
            points = np.load(self.sample_sequence_list[index])
        elif self.sample_sequence_list[index].endswith('.pcd'):
            # from pyntcloud import PyntCloud
            # path = str(self.sample_sequence_list[index])
        ...
        points = np.insert(points, 3, values=0, axis=1)
        points = np.insert(points, 4, values=0, axis=1)
