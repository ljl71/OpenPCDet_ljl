...
from pcdet.config_pvrcnnpp_ld import cfg, cfg_from_yaml_file
...
class LoadModel_pvrcnnpp_ld:
    def __init__(self, ckp_path = "/workspace/OpenPCDet/checkpoints/pvrcnnpp_liangdao.pth", cfg_file = "/workspace/OpenPCDet/tools/cfgs/custom_models/pv_rcnn_plusplus_liangdao.yaml"):
    ...
    async def inference(self, FilePath, shape=4):
        ...
        for a in range(len(det_annos)):
            for i in range(len(det_annos[a]["name"])):
                direct = det_annos[a]["boxes_lidar"][i][6]
                # if direct < 4:
                #     direct += math.pi
