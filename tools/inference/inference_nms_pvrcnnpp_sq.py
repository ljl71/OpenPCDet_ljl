...
from pcdet.config_pvrcnnpp_sq import cfg, cfg_from_yaml_file
...
class LoadModel_pvrcnnpp_sq:
    def __init__(self, ckp_path = "/workspace/OpenPCDet/checkpoints/checkpoint_epoch_98.pth", cfg_file = "/workspace/OpenPCDet/tools/cfgs/custom_models/pv_rcnn_plusplus.yaml"):
    ...
    async def inference(self, FilePath, shape=4):
        ...
        for a in range(len(det_annos)):
            for i in range(len(det_annos[a]["name"])):
                direct = det_annos[a]["boxes_lidar"][i][6] - math.pi / 4
                # if direct < 4:
                #     direct += math.pi
                # while direct > math.pi / 2:
                #     direct = direct - math.pi / 2
                print(det_annos[a]["boxes_lidar"][i][6])
                print(direct)
                dicObj = {
                    "type": "shape",
                    "drawType": "box3d",
